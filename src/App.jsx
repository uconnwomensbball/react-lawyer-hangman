  ...
  <div className="font-bzks bg-slate-50 flex flex-col items-center p-6 min-h-screen">
    ...
    {gameStarted && (
      ...
      {caseCheckpoints.map((checkpoint, index) => (
        ...
        <section className="flex flex-col justify-center items-center mb-2 sm:mb-8 gap-2">
        <div className="flex flex-row gap-2 mt-2 sm:mb-6 sm:h-10 text-xs sm:text-sm flex-wrap justify-center">
        {!isGameOver && randomWord.word.split("").map((letter, index)=> <span key={index} className={`px-2 py-1 sm:px-4 sm:py-2 w-10 bold sm:text-xl ${letter === " "? "":"border-b-2"}`}>{guessedLetters.includes(letter)? letter.toUpperCase(): " "}</span>)}
        {isGameOver && randomWord.word.split("").map((letter, index)=> <span key={index} className={`px-2 py-1 sm:px-4 sm:py-2 sm:text-xl ${letter === " "? "":"border-b-2"}`}><p className={!guessedLetters.includes(letter) && randomWord.word.includes(letter)? "text-red-500": ""}>{letter.toUpperCase()}</p></span>)}
        </div>
        <p className="sm:text-xl text-center text-xs mt-2 mb-2">Hint: {randomWord.definition}</p>
      </section>
    ...
    {!gameStarted && isInitialScreenDisplayed && (<div className="flex flex-col justify-center items-center min-h-screen">
       <FontAwesomeIcon icon={faGavel} className="text-3xl sm:text-7xl"></FontAwesomeIcon>
      <h1 className="sm:text-4xl sm:mb-6 mb-4 mt-4 tracking-wide font-medium">Lawyer Hangman</h1>
      <h2 className="text-xs text-center sm:text-xl tracking-wide">Would you like to guess legal terms related to criminal law or civil law? </h2>
